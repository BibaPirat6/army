            ...
            <table class="border-collapse text-xs">
                <thead>
                    <tr>
                        ...
                        @foreach($tasks as $t)
                            <th class="sticky top-0 z-20 px-1.5 py-1.5 text-center border-b border-gray-200 group" style="background-color: {{ $t->color }}10; min-width: 75px; width: 75px;">
                                <a href="{{ route('calendar.show', $t->id) }}" class="flex flex-col items-center gap-0.5 hover:opacity-70 relative" title="{{ $t->title }}">
                                    <span class="w-2 h-2 rounded-full" style="background: {{ $t->color }}"></span>
                                    <span class="truncate w-full">{{ Str::limit($t->title, 8) }}</span>
                                    @if($resp = $t->employeePosition?->employee?->person)
                                        <span class="absolute -bottom-3 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-[9px] px-1 py-0.5 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 z-40 pointer-events-none">
                                            {{ $resp->фамилия }} {{ mb_substr($resp->имя, 0, 1) }}.
                                        </span>
                                    @endif
                                </a>
                            </th>
                            ...
                            @foreach($tasks as $t)
                                ...
                                <td class="relative px-1.5 py-1.5 text-center border-b group" style="min-width: 75px; width: 75px;">
                                    ...
                                    @else
                                        ...
                                        <a href="{{ route('calendar.show', $t->id) }}#assign"
