...
@section('content')
    <div class="px-3 py-3">
        ...
        <div class="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
                <a href="{{ route('calendar.index') }}"
                    class="text-indigo-600 hover:text-indigo-800 flex items-center gap-2 mb-2">
        ...
        @else
                        ...
                        <tr class="bg-gray-50">
                            ...
                            @foreach ($tasks as $t)
                                <th class="task-column sticky top-0 z-30 border-b border-gray-200 group relative"
                                    ...
                                    <div
                                        class="resize-handle absolute right-0 top-0 h-full w-1 cursor-col-resize hover:bg-indigo-400">
                                    </div>
                                    <a href="{{ route('calendar.tasks.show', $t->id) }}"
                                        ...
                                        <div class="flex items-center gap-1.5 w-full">
                                            ...
                                            <span
                                                class="line-clamp-2 text-left text-[11px] font-semibold leading-4 text-gray-700">{{ $t->title }}</span>
                                        </div>
                    ...
                    <tbody>
                        @foreach ($matrix as $row)
                            ...
                            <tr class="hover:bg-gray-50/70 transition">
                                <td class="sticky left-0 z-20 border-b border-r border-gray-100 bg-white px-3 py-2"
                                    style="min-width: 200px;">
                                    <div class="flex flex-col">
                                        <div class="flex items-center gap-2">
                                            <div
                                                class="w-7 h-7 rounded-full bg-gradient-to-r from-indigo-100 to-indigo-200 flex items-center justify-center text-indigo-700 font-semibold text-xs flex-shrink-0">
                                                {{ mb_substr($name, 0, 1) }}
                                            </div>
                                            <div>
                                        ...
                                        @if ($cp)
                                            <a href="{{ route('calendar.schedule.employee', $e->id) }}"
                                                class="inline-flex items-center gap-1.5 mt-1.5 px-3 py-1.5 
          ...
          hover:shadow-sm hover:shadow-emerald-100"
                                                ...
                                                <svg class="w-3 h-3 flex-shrink-0 transition-transform duration-200 group-hover:translate-x-0.5"
                                ...
                                @foreach ($tasks as $t)
                                    ...
                                    <td class="border-b border-gray-100 px-1 py-1 align-top"
                                        ...
                                        @if ($a)
                                            ...
                                            <a href="{{ route('calendar.assignments.edit', [$t->id, $a->id]) }}"
                                                class="group block rounded-lg border border-gray-100 bg-white p-3 shadow-sm transition hover:shadow-md hover:border-gray-200">

                                                <div class="mb-2 flex items-center justify-between">
                                                    {{-- Счетчик задач --}}
                                                    <div
                                                        class="text-xs font-bold {{ $pct >= 100 ? 'text-emerald-600' : ($pct > 50 ? 'text-indigo-600' : 'text-amber-600') }}">
                                                        {{ $a->completed_count }}/{{ $a->quota }}
                                                    </div>

                                                    {{-- Иконка огонька с цифрой приоритета --}}
                                                    <div class="relative flex-shrink-0">
                                                        {{-- Огонек --}}
                                                        <svg class="w-7 h-7 text-orange-500 drop-shadow-sm"
                                                            fill="currentColor" viewBox="0 0 24 24">
                                                            <path
                                                                d="M12 23c-4.4 0-8-3.6-8-8 0-3.7 2.1-7.3 5-9.7C10.4 3.7 11.2 3 12 3s1.6.7 3 2.3c2.9 2.4 5 6 5 9.7 0 4.4-3.6 8-8 8z" />
                                                            <path
                                                                d="M12 19c-2.2 0-4-1.8-4-4 0-2.5 1.8-4.8 3.2-6 .4-.4 1.2-.4 1.6 0 1.4 1.2 3.2 3.5 3.2 6 0 2.2-1.8 4-4 4z"
                                                                class="text-orange-300" />
                                                        </svg>
                                                        {{-- Цифра внутри огонька --}}
                                                        <span
                                                            class="absolute inset-0 flex items-center justify-center text-xs font-extrabold text-white"
                                                ...
                                                {{-- Прогресс-бар --}}
                                                <div class="mb-1.5 h-1.5 overflow-hidden rounded-full bg-gray-100">
                                                    <div class="h-full rounded-full transition-all {{ $pct >= 100 ? 'bg-emerald-500' : ($pct > 50 ? 'bg-indigo-500' : 'bg-amber-500') }}"
                                        ...
                                        @else
                                            <div
                                                class="group relative flex min-h-[70px] items-center justify-center rounded-lg border border-dashed border-gray-200 bg-gray-50 hover:border-indigo-300 hover:bg-indigo-50 transition">
                                                ...
                                                <a href="{{ route('calendar.assignments.create', [$t->id, $e->id]) }}"
                                                    ...
                                                    <span
                                                        class="rounded-lg bg-indigo-500 px-2 py-1 text-[10px] font-medium text-white shadow hover:bg-indigo-600 whitespace-nowrap">
                                                        Назначить
                                                    </span>
                                                </a>
                                            </div>
    ...
    <script>
        ...
        function saveWidth(column, width) {
            savedWidths[column] = width;
            localStorage.setItem('matrixWidths', JSON.stringify(savedWidths));
        }

        function getWidth(column, defaultWidth) {
            return savedWidths[column] || defaultWidth;
        }
        ...
        function applyScale() {
            ...
            if (empCol) {
                empCol.style.minWidth = empWidth + 'px';
                empCol.style.width = empWidth + 'px';
            }
            document.querySelectorAll('#matrixTable tbody td:first-child').forEach(td => {
                td.style.minWidth = empWidth + 'px';
                td.style.width = empWidth + 'px';
            });
            document.querySelectorAll('.task-column').forEach((col, idx) => {
                ...
                document.querySelectorAll(`#matrixTable tbody tr td:nth-child(${idx + 2})`).forEach(td => {
        ...
        function changeScale(diff) {
            currentScale = Math.min(180, Math.max(60, currentScale + diff));
            applyScale();
        }
