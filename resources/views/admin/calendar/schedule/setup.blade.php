    ...
    <div class="max-w-full mx-auto px-6 py-4">
        ...
        <script>
            ...
            function addBreak(day) {
                ...
                d.innerHTML =
                    `<input type="time" name="days[${day}][breaks][${breakCounter[day]}][start]" value="13:00" class="break-input w-full text-[11px] rounded border-gray-200 py-0.5 px-1.5 focus:border-indigo-400 focus:ring-0 outline-none"><span class="text-gray-300 text-[11px]">—</span><input type="time" name="days[${day}][breaks][${breakCounter[day]}][end]" value="14:00" class="break-input w-full text-[11px] rounded border-gray-200 py-0.5 px-1.5 focus:border-indigo-400 focus:ring-0 outline-none"><button type="button" onclick="removeBreak(this)" class="text-gray-300 hover:text-red-500 text-sm leading-none">&times;</button>`;
                c.appendChild(d);
                recalc();
            }

            function removeBreak(b) {
                b.closest('.break-row').remove();
                recalc();
            }
            ...
            function recalc() {
                ...
                document.querySelectorAll('.day-card').forEach(card => {
                    ...
                    if (type === 'рабочий_день') {
                        ...
                        if (v !== true) {
                            ...
                            if (sum) sum.textContent = 'ошибка';
                            return;
                        }
                        card.classList.remove('border-red-300');
                        const m = calcDay(card);
                        w += m;
                        if (sum) sum.textContent = (m / 60).toFixed(1) + ' ч';
                    } else {
                        det?.classList.add('hidden');
                        card.classList.add('border-gray-100', 'bg-gray-50');
                        card.classList.remove('border-gray-200', 'border-red-300');
                        if (sum) sum.textContent = 'выходной';
                    }
                });
                const h = w / 60,
                    t = parseInt(document.getElementById('weeklyHoursTotal').value) || 40,
                    d = h - t,
                    da = Math.abs(d);
                ...
                if (err) {
                    ge.textContent = '⚠️ ' + msg;
                    ge.classList.remove('hidden');
                    sb.classList.add('hidden');
                    btn.disabled = true;
                } else if (da < 0.05) {
                    ge.classList.add('hidden');
                    sb.className = 'px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700';
                    sb.textContent = '✓ Готово';
                    sb.classList.remove('hidden');
                    btn.disabled = false;
                } else if (d < 0) {
                    ge.textContent = '⚠️ Недобор ' + da.toFixed(1) + ' ч';
                    ge.classList.remove('hidden');
                    sb.classList.add('hidden');
                    btn.disabled = true;
                } else {
                    ge.textContent = '⚠️ Перебор +' + da.toFixed(1) + ' ч';
                    ge.classList.remove('hidden');
                    sb.classList.add('hidden');
                    btn.disabled = true;
                }
            ...
            document.addEventListener('input', e => {
                if (e.target.closest('.day-card')) recalc();
            });
            recalc();
        </script>
    @endsection
