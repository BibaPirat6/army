        ...
        <div class="bg-white rounded-lg shadow-lg">
            ...
            <div class="px-6 py-4 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-gray-800">{{ $task->end_date ? $task->end_date->format('d.m.Y') : 'Не указана' }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex items-center"><div class="w-6 h-6 rounded border" style="background-color:{{ $task->color }}"></div><span class="ml-2 text-gray-800">{{ $task->color }}</span></div>
                    </div>
                </div>
                        ...
                        <div class="space-y-1">
                            @foreach($task->files as $file)
                                <div class="flex items-center justify-between text-sm">
                                    <a href="{{ asset('storage/'.$file->path) }}" target="_blank" class="text-indigo-600 hover:underline">📎 {{ $file->original_name }}</a>
            ...
            <div class="border-t border-gray-200 px-6 py-4">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button onclick="openSubtaskModal()" class="px-3 py-1 text-sm text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition">+ Добавить</button>
                </div>
                <div class="space-y-2">
                    @forelse($task->subtasks as $s)
                        <div class="bg-gray-50 rounded-lg p-3 flex justify-between items-center">
                            <div>
                                ...
                                <p class="text-xs text-gray-500">мин {{ $s->min_time_minutes }} | сред {{ $s->avg_time_minutes }} | макс {{ $s->max_time_minutes }} мин</p>
                            </div>
                            <div class="flex gap-2">
                                <button onclick="editSubtask({{ $s->id }})" class="text-indigo-600 hover:text-indigo-800">✏️</button>
                                <button onclick="deleteSubtask({{ $s->id }})" class="text-red-600 hover:text-red-800">🗑️</button>
                            </div>
                        </div>
                    @empty
                        <div class="bg-red-50 border border-red-200 rounded-lg p-4 text-sm text-red-600">
                            ⚠️ Нет подзадач. <button onclick="openSubtaskModal()" class="underline font-medium">Добавить</button>
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="border-t border-gray-200 px-6 py-4" id="assign">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-medium text-gray-800">Назначенные сотрудники ({{ $task->taskAssignments->count() }})</h3>
                ...
                @forelse($task->taskAssignments as $a)
                    ...
                    <div class="bg-gray-50 rounded-lg p-3 flex items-center justify-between mb-2">
                        <div class="flex items-center gap-3">
                            <a href="{{ route('employees.show', $a->employee_id) }}" class="font-medium text-indigo-600 hover:underline">{{ $ename }}</a>
                            <span class="text-xs text-gray-500">P{{ $a->priority }}</span>
                            <span class="text-xs text-gray-400">{{ $a->start_date?->format('d.m.Y') ?? '—' }} — {{ $a->end_date?->format('d.m.Y') ?? '—' }}</span>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="text-sm">{{ $a->completed_count }}/{{ $a->quota }}</span>
                            <div class="w-24 bg-gray-200 rounded-full h-2"><div class="h-2 rounded-full {{ $pct>=100?'bg-emerald-500':'bg-indigo-500' }}" style="width:{{ $pct }}%"></div></div>
                            <a href="{{ route('calendar.assignments.edit', [$task->id, $a->id]) }}" class="text-xs text-indigo-600 hover:bg-indigo-50 rounded px-2 py-1">✏️</a>
                            <form action="{{ route('calendar.assignments.destroy', [$task->id, $a->id]) }}" method="POST" onsubmit="return confirm('Удалить?')" class="inline">
                                @csrf @method('DELETE')
        ...
        <div class="flex items-center justify-center min-h-full p-4">
            <div class="relative bg-white rounded-lg shadow-xl w-full max-w-lg p-6">
                ...
                <form id="editTaskForm" enctype="multipart/form-data">
                    @csrf @method('PUT')
                    <div class="mb-4"><label class="block text-sm font-medium text-gray-700 mb-1">Название *</label><input type="text" name="title" id="edit_title" value="{{ $task->title }}" required class="w-full rounded-lg border-gray-300"></div>
                    <div class="mb-4"><label class="block text-sm font-medium text-gray-700 mb-1">Описание</label><textarea name="description" id="edit_description" rows="2" class="w-full rounded-lg border-gray-300">{{ $task->description }}</textarea></div>
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Цвет</label><input type="color" name="color" id="edit_color" value="{{ $task->color }}" class="h-10 w-full rounded-lg border-gray-300 cursor-pointer"></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Квота</label><input type="number" name="quota" id="edit_quota" value="{{ $task->quota }}" min="1" class="w-full rounded-lg border-gray-300"></div>
                    </div>
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Дата начала</label><input type="date" name="start_date" id="edit_start_date" value="{{ $task->start_date->format('Y-m-d') }}" class="w-full rounded-lg border-gray-300"></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Дата окончания</label><input type="date" name="end_date" id="edit_end_date" value="{{ $task->end_date?->format('Y-m-d') }}" class="w-full rounded-lg border-gray-300"></div>
                    </div>
                    <div class="mb-4">
                    ...
                    <div class="flex justify-end gap-2">
                        <button type="button" onclick="closeEditTaskModal()" class="px-4 py-2 text-sm text-gray-700 bg-gray-100 rounded-lg">Отмена</button>
                        <button type="submit" class="px-4 py-2 text-sm text-white bg-indigo-600 rounded-lg">Сохранить</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="flex items-center justify-center min-h-full p-4">
            <div class="relative bg-white rounded-lg shadow-xl w-full max-w-md p-6">
                ...
                <form id="subtaskForm">
                    ...
                    <div class="mb-4"><label class="block text-sm font-medium text-gray-700 mb-1">Название *</label><input type="text" id="subtask_title" required class="w-full rounded-lg border-gray-300"></div>
                    <div class="grid grid-cols-3 gap-3 mb-4">
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Мин</label><input type="number" id="subtask_min" required min="0" class="w-full rounded-lg border-gray-300"></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Сред</label><input type="number" id="subtask_avg" required min="0" class="w-full rounded-lg border-gray-300"></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Макс</label><input type="number" id="subtask_max" required min="0" class="w-full rounded-lg border-gray-300"></div>
                    </div>
                    <div class="flex justify-end gap-2">
                        <button type="button" onclick="closeSubtaskModal()" class="px-4 py-2 text-sm text-gray-700 bg-gray-100 rounded-lg">Отмена</button>
                        <button type="submit" class="px-4 py-2 text-sm text-white bg-indigo-600 rounded-lg">Сохранить</button>
                    </div>
                </form>
            </div>
