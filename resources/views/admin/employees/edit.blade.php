                        ...
                        <div class="grid grid-cols-4 gap-4">
                            @foreach ($columns as $column)
                                @php
                                    ...
                                    // Для JSON полей преобразуем в строку для hidden input
                                    $jsonValueForInput = '';
                                    if ($isJson && $currentValue) {
                                    ...
                                    $dateValue = '';
                                    if ($isDate && !empty($currentValue)) {
                                        try {
                                            $dateValue = \Carbon\Carbon::parse($currentValue)->format('Y-m-d');
                                        } catch (\Exception $e) {
                                            $dateValue = $currentValue;
                                        }
                                    }
                                    
                                    // Для decimal/float полей форматируем значение
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.json-list-container').forEach(container => {
            const hiddenInput = container.querySelector('.json-hidden-text');
            const tagsContainer = container.querySelector('.json-tags-list');
            
            if (hiddenInput && tagsContainer && hiddenInput.value) {
                const values = hiddenInput.value.split('\n').filter(v => v.trim() !== '');
                if (values.length === 0 && tagsContainer.children.length === 0) {
                    tagsContainer.innerHTML = '<span class="text-gray-400 text-sm italic">Нет данных</span>';
                }
            }
        });
