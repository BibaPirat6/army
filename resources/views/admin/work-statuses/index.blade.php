...
@section('content')
    ...
    <div class="max-w-4xl mx-auto p-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
            <div>
                <h1 class="text-2xl font-bold text-[#060606]">Рабочие статусы</h1>
                <p class="text-[#565A5B] mt-1">Управление рабочими статусами сотрудников</p>
            </div>
            <a href="{{ route('work-statuses.create') }}"
                ...
                <table class="w-full">
                    <thead class="bg-[#565A5B]">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-sm font-semibold text-[#e7e1e1]">Описание</th>
                            <th class="px-6 py-4 text-left text-sm font-semibold text-[#e7e1e1]">На английском</th>
                    ...
                    <tbody class="divide-y divide-[#BFBFBF]">
                        @forelse($statuses as $status)
                            <tr class="hover:bg-[#A60644]/5 transition-colors duration-200">
                                <td class="px-6 py-4 text-[#060606] font-medium">{{ $status->id }}</td>
                                <td class="px-6 py-4 text-[#060606]">{{ $status->description }}</td>
                                <td class="px-6 py-4 text-[#060606] font-mono text-sm bg-[#060606]/5 rounded px-2 py-1">
                                    {{ $status->name }}</td>
                                <td class="px-6 py-4 text-right space-x-2">
                                    <a href="{{ route('work-statuses.edit', $status->id) }}"
                                    ...
                                    <form action="{{ route('work-statuses.delete', $status->id) }}" method="post"
                                        class="inline-block"
                                        onsubmit="return confirm('Вы уверены, что хотите удалить статус \"{{ $status->description }}\"?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-[#565A5B] text-lg font-medium">Нет рабочих статусов</p>
                                        <p class="text-[#7F7F7F] mt-1">Создайте первый рабочий статус для начала работы</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
